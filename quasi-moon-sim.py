...
from matplotlib.widgets import Button, Slider, CheckButtons     # Used for buttons and sliders in a pyplot
...
class Body:
    ...
    NAMES = {0: "sun", 1: "mercury", 2: "venus", 3: "earth", 4: "mars", 5: "jupiter", 6: "saturn", 7: "uranus",
             8: "neptune", 524522: "zoozve"}
    COLORS = {0: "xkcd:goldenrod", 1: "xkcd:grey", 2: "xkcd:dark orange", 3: "xkcd:clear blue", 4: "xkcd:rust",
              5: "xkcd:light brown", 6: "xkcd:tan", 7: "xkcd:sea blue", 8: "xkcd:grey blue", 524522: "xkcd:neon purple"}
    MASSES = {0: 1.989e30, 1: 3.301e23, 2: 4.868e24, 3: 5.972e24, 4: 6.417e23, 5: 1.898e27, 6: 5.683e26, 7: 8.681e25,
              8: 1.024e26, 524522: 10e14}   # The mass for 2002 VE68 is approximated, which seems to function
...
class System:
    ...
    euler()
        Computes the new position and velocity using the Euler method
    verlet()
        Computes the new position and velocity using the position Verlet method
    update()
        Updates the simulation for one time interval h

    Notes
    -----
    The additional approximation methods can be used by replacing the function called in the update() function without
    any other issues
    """
    ...
    def __init__(self, n: int, h: float, year: int):
        ...
        self.bodies = np.array([], dtype=Body)
        for i in range(self.n + 1):
            self.bodies = np.append(self.bodies, Body(i, self.year))
        self.bodies = np.append(self.bodies, Body(524522, self.year))
    ...
    def update(self):
        ...
        for nasa_id in range(self.bodies.size):
            self.rk4(nasa_id)
...
ax_ran = fig.add_axes([0.10, 0.05, 0.10, 0.03])
...
ran_button = Button(ax_ran, 'Random', hovercolor="xkcd:clear blue")
...
while can_run is False:
    ...
    ran_button.on_clicked(allow_ran)
    run_button.on_clicked(run)
    plt.pause(0.1)

# Add or remove velocity randomly to deviate 2002 VE68
if randomize:
    solar.bodies[-1].velocity = solar.bodies[-1].velocity + np.array([random.uniform(-0.02, 0.02),
